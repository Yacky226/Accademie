...
export class SubmissionsService {
  ...
  async listMySubmissions(requesterId: string): Promise<SubmissionEntity[]> {
    const requester = await this.submissionsRepository.findUserById(requesterId);
    if (!requester) {
  ...
  async listSubmissionsByProblem(problemId: string): Promise<SubmissionEntity[]> {
  ...
  async getSubmissionById(submissionId: string): Promise<SubmissionEntity> {
    const submission = await this.submissionsRepository.findSubmissionById(submissionId);
    if (!submission) {
  ...
  async createSubmission(requesterId: string, dto: CreateSubmissionDto): Promise<SubmissionEntity> {
    const requester = await this.submissionsRepository.findUserById(requesterId);
    if (!requester) {
      throw new NotFoundException('Requester user not found');
    }
    ...
    if (dto.problemId) {
      const problem = await this.submissionsRepository.findProblemById(dto.problemId);
      if (!problem) {
        throw new NotFoundException('Problem not found');
      }
    ...
    if (dto.languageId) {
      const language = await this.submissionsRepository.findLanguageById(dto.languageId);
      if (!language) {
    ...
    if (dto.judgeRunId) {
      const judgeRun = await this.submissionsRepository.findJudgeRunById(dto.judgeRunId);
      if (!judgeRun) {
        throw new NotFoundException('Judge run not found');
      }
      this.applyJudgeRunResult(submission, judgeRun);
    } else if (submission.language) {
      const judgeRun = new JudgeRunEntity();
      judgeRun.token = randomUUID();
      judgeRun.sourceCode = submission.sourceCode;
      judgeRun.stdin = submission.stdin;
      judgeRun.expectedOutput = submission.expectedOutput;
      judgeRun.status = SubmissionStatus.PENDING;
      judgeRun.verdict = 'PENDING';
      judgeRun.requester = requester;
      judgeRun.problem = submission.problem;
      judgeRun.language = submission.language;
      submission.judgeRun = await this.submissionsRepository.saveJudgeRun(judgeRun);
    }
  ...
  async updateSubmissionResult(
    ...
    submission.maxScore = dto.maxScore !== undefined ? dto.maxScore.toFixed(2) : submission.maxScore;
    submission.score = dto.score !== undefined ? dto.score.toFixed(2) : submission.score;
    ...
    if (submission.score === undefined && submission.verdict) {
      submission.score = this.computeScoreFromVerdict(submission.verdict).toFixed(2);
    }
  ...
  private inferVerdict(submission: SubmissionEntity): string {
    ...
    if (submission.expectedOutput !== undefined && submission.stdout !== undefined) {
  ...
  private applyJudgeRunResult(submission: SubmissionEntity, judgeRun: JudgeRunEntity): void {
    ...
    if (judgeRun.status !== SubmissionStatus.PENDING && judgeRun.status !== SubmissionStatus.RUNNING) {
      submission.evaluatedAt = new Date();
      submission.score = this.computeScoreFromVerdict(judgeRun.verdict).toFixed(2);
    }
  }
}
